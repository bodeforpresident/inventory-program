...
void tableOutputUnsorted(vector<string>& resistors, vector<int>& resistorValues, int& loop); // display table
...
int main() {
	...
	do {
		...
		if (userStartChoice == 1) {
			do {
				loop = 1;
				tableOutputUnsorted(resistors, resistorValues, loop);
			} while (loop == 1);
		}
	} while (loop == 3);
}

void tableOutputUnsorted(vector<string>& resistors, vector<int>& resistorValues, int& loop) { // table output
	int k = 0;
	...
	do {
		if (resistors[k].length() == 5 && resistorValues[k] < 10) {
			...
			k = resistors.size() + 1;
			// adding and removing
			cout << "\nPress 1 to add resistors, press 2 to remove resistors, press 3 to quit back to the main menu: ";
			...
			if (userResistorChoice == 1) {
				...
				cin >> userResistorChoice;
				cout << "Enter how many resistors you want to remove: ";
				cin >> userResistorAmount;

				resistorValues[userResistorChoice - 1] = resistorValues[userResistorChoice - 1] - userResistorAmount;
			} else if (userResistorChoice == 3) {
				loop = 3;
			}
		}
